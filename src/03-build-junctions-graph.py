...
def name_junctions(lower_level_graph, nodes_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    junction_names = (
        ox.graph_to_gdfs(lower_level_graph, nodes=False)
        .reset_index()[["u", "name"]]
        .fillna("")
    )
    ...
    nodes_df = nodes_df.merge(
        junction_names, how="left", left_on="junction_id", right_on="u"
    )

    cluster_names = (
        nodes_df.groupby("junction_cluster_id")["name"]
        .apply(combine_names)
        .reset_index()
    )

    nodes_df = nodes_df.merge(
        cluster_names, how="left", on="junction_cluster_id", suffixes=["", "_cluster"]
    )

    nodes_df["junction_cluster_name"] = nodes_df["name_cluster"].apply(
        list_to_string_name
    )

    nodes_df["junction_cluster_name"] = nodes_df["junction_cluster_name"].apply(
        shorten_road_names
    )

    nodes_df["name_rank"] = nodes_df.groupby(["junction_cluster_name"])[
        "junction_cluster_id"
    ].transform("rank", method="dense")

    nodes_df["name_max_rank"] = nodes_df.groupby(["junction_cluster_name"])[
        "name_rank"
    ].transform("max")
    ...
    nodes_df["junction_cluster_name"] = np.where(
        ...
        nodes_df["junction_cluster_name"]
        + "-"
        + nodes_df["name_rank"].astype(int).astype(str),
    )
...
def main():
    ...
    df_higher = ox.graph_to_gdfs(
        G2, nodes=True, edges=False, node_geometry=True, fill_edge_geometry=False
    )
    ...
    # Combine datasets
    df = df_lower.merge(
        df_higher, how="inner", on="osmid_original", suffixes=["_original", "_cluster"]
    )

    # calculate lat, lons for clusters
    cluster_coords = (
        df.groupby("osmid_cluster")[["lat", "lon"]]
        .mean()
        .reset_index()
    ...
    # join in
    df = df.merge(cluster_coords, how="left", on="osmid_cluster")
    ...
    df = (
        df.drop(columns=["geometry_original", "geometry_cluster"])
        .reset_index()
        .rename(
            columns={
