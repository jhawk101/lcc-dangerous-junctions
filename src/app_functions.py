...
def combine_junctions_and_collisions(
    ...
    junction_collisions = junctions.merge(
        collisions,
...
def get_all_year_df(junction_collisions: pd.DataFrame) -> pd.DataFrame:
    ...
    all_clusters = (
        junction_collisions[["junction_cluster_id"]].dropna().drop_duplicates()
    )
    all_years["key"] = 0
    all_clusters["key"] = 0
...
def calculate_metric_trajectories(
    junction_collisions: pd.DataFrame, dangerous_junctions: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    yearly_stats = (
        filtered_junction_collisions.groupby(["junction_cluster_id", "year"])[
            "danger_metric"
        ]
        .sum()
        .reset_index()
    ...
    yearly_stats = (
        all_years_mapping.merge(
            yearly_stats, how="left", on=["year", "junction_cluster_id"]
        )
        .fillna(0)
    ...
    dangerous_junctions = dangerous_junctions.merge(
        yearly_stats, how="left", on="junction_cluster_id"
    )
...
def calculate_dangerous_junctions(
    junction_collisions: pd.DataFrame, n_junctions: int, casualty_type: str
) -> pd.DataFrame:
    """
    ...
    dangerous_junctions = (
        junction_collisions.groupby(grp_cols)[agg_cols]
        .sum()
        .reset_index()
        .sort_values(
            by=["recency_danger_metric", f"fatal_{casualty_type}_casualties"],
    ...
    dangerous_junctions = calculate_metric_trajectories(
        junction_collisions, dangerous_junctions
    )

    dangerous_junctions["label"] = dangerous_junctions.apply(
        lambda row: create_junction_labels(row, casualty_type), axis=1
    )
...
@st.cache_data(show_spinner=False, ttl=3 * 60, max_entries=5)
def get_low_level_junction_data(
    junction_collisions: pd.DataFrame, chosen_point: list
) -> pd.DataFrame:
    """
...
def get_map_bounds(top_dangerous_junctions: pd.DataFrame) -> list:
    ...
    """
    sw = (
        top_dangerous_junctions[["latitude_cluster", "longitude_cluster"]]
        .min()
        .values.tolist()
    )
    ne = (
...
@st.cache_data(show_spinner=False, ttl=3 * 60, max_entries=5)
def get_most_dangerous_junction_location(
    first_row_dangerous_junctions: pd.DataFrame,
) -> list:
    """
    Slight hack to make sure the low level map only updates when the first row of data changes
    """
    location = first_row_dangerous_junctions[
...
def get_high_level_fg(
    dangerous_junctions: pd.DataFrame, map_data: pd.DataFrame, n_junctions: int
) -> folium.FeatureGroup:
    """
...
def get_low_level_fg(
    dangerous_junctions: pd.DataFrame,
    ...
    for id, lat, lon, junction_rank in dangerous_junctions[cols].values:
        ...
        id_collisions = junction_collisions[
            junction_collisions["junction_cluster_id"] == id
        ]
        ...
        for collision_lat, collision_lon, severity, label in (
            id_collisions[cols].dropna().values
        ):
        ...
        fg.add_child(
            folium.CircleMarker(location=[lat, lon], radius=10, fill_opacity=1)
        )
