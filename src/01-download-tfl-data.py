...
def clean_collision_id(raw_collision_id: str, year: int, borough: str) -> int:
    ...
    if year < 2017:
        collision_id = (
            str(year) + str(raw_collision_id)[0:2] + str(raw_collision_id)[-7:]
        )
    ...
    with requests.Session() as session:
        for link in links:
            ...
            while len([c for c in cols if "Unnamed:" in c]) > 0:
                df = pd.read_csv(
                    StringIO(
                        download.content.decode(encoding="utf-8", errors="replace")
                    ),
...
def main():
    ...
    casualty_links = [
        ...
        if ("casualty" in link) or ("casualties" in link)
    ]

    # ====================== COLLISIONS ===================================== #

    collisions = process_yearly_data(collision_links, collision_cols, column_aliases)

    collisions["date"] = pd.to_datetime(
        collisions["date"], format="mixed", dayfirst=True
    )
    ...
    collisions["collision_id"] = collisions.apply(
        lambda row: clean_collision_id(
            row["raw_collision_id"], row["year"], row["borough"]
        ),
        axis=1,
    )
    ...
    collisions["longitude"], collisions["latitude"] = convert_lonlat(
        collisions["easting"], collisions["northing"]
    )
    ...
    casualties = process_yearly_data(casualty_links, casualty_cols, column_aliases)
