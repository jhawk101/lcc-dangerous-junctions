...
def recalculate_severity(casualties, mode_of_travel):
    ...
    recalculated_severities = (
        casualties[casualties["casualty_type"] == mode_of_travel]
    ...
    recalculated_severities[new_cols] = pd.DataFrame(
        recalculated_severities[0].tolist(), index=recalculated_severities.index
    )

    recalculated_severities[f"max_{casualty_type}_severity"] = (
        recalculated_severities.apply(
...
def main():
    ...
    collisions = pd.read_parquet("data_dft/collisions.pq")
    casualties = pd.read_parquet("data_dft/casualties.pq")
    ...
    print(
        f"Filter to cyclist & pedestrian collisions, {len(valid_crash_ids)} crash IDs in data"
    )
    ...
    collisions = collisions.merge(
        recalculated_cyclist_severities, how="left", on="accident_index"
    ...
    collisions.loc[
        ~collisions["max_cyclist_severity"].isnull(), "is_cyclist_collision"
    ] = True
    collisions.loc[
        ~collisions["max_pedestrian_severity"].isnull(), "is_pedestrian_collision"
    ] = True
    ...
    # output csvs
    print("Output to pq")
    collisions.to_parquet("data_dft/pedestrian-and-cyclist-collisions.pq", index=False)
